export interface ParsedB2CTopupResult {
  resultCode: string;
  resultDesc: string;
...
/**
 * Extracts B2B/B2C transaction key-value parameters from standard Safaricom Callback structures.
 */
export function parseB2CTopupResult(payload: any): ParsedB2CTopupResult {
  const result = payload?.Result;
  if (!result) {
    throw new Error('Invalid webhook payload structure: Missing Result object.');
  }

  const parsed: ParsedB2CTopupResult = {
